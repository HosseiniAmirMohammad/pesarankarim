...
check(
    "دکمه «✅ نظر دادم» زیر گیف نیست (بعداً در پیام جدا ارسال می‌شود)",
    gif_call.get("reply_markup") is None,
    str(gif_call.get("reply_markup")),
)
check(
    "دکمه کیبورد «✅ نظر دادم» در پیام بعدی ارسال شد",
    any(
        keyboard_texts(m) == ["✅ نظر دادم"]
...
check(
    "بعد از بله، منوی اصلی ارسال نمی‌شود (بعد از کلیک دکمه ارسال خواهد شد)",
    not any(
