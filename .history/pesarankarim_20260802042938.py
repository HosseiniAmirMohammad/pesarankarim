...
BTN_MASHHAD = InlineKeyboardButton("شعبه مشهد (خیام)", callback_data="mashhad")
BTN_TEHRAN = InlineKeyboardButton("شعبه تهران", callback_data="tehran")

def membership_kb():
    return make_keyboard([BTN_JOIN,BTN_CHECK], row_width=1)

def branch_kb():
    return make_keyboard([BTN_MASHHAD,BTN_TEHRAN], row_width=2)
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(branch_callback, pattern="branch_"))
