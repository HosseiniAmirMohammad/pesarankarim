...
async def admin_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "➕ افزودن ادمین جدید\n\n" "" "",
        reply_markup=ReplyKeyboardMarkup[[BTN_ADMIN_BACK]],
...
async def admin_remove(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for admin_id in admins:
        if admin_id != update.effective_user.id:
            keyboard.append([KeyboardButton(str(admin_id))])
...
async def handle_admin_management(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("admin_action") == "add_admin":
        ...
        try:
            ...
            if add_admin(new_admin_id):
                await update.message.reply_text(
                    f"✅ ادمین با آیدی {new_admin_id} با موفقیت اضافه شد."
                )
            else:
                await update.message.reply_text(
                    f"❌ کاربر با آیدی {new_admin_id} قبلاً ادمین است."
                )
        except ValueError:
    ...
    if text.isdigit() and context.user_data.get("admin_action") == "remove_admin":
        ...
        if remove_admin(admin_id_to_remove):
            await update.message.reply_text(
                f"✅ ادمین با آیدی {admin_id_to_remove} با موفقیت حذف شد."
            )
        else:
            await update.message.reply_text(
                f"❌ کاربر با آیدی {admin_id_to_remove} ادمین نیست."
            )
