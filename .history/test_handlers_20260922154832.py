...
callbacks = re.findall(
    r'CallbackQueryHandler\s*\(\s*(\w+)\s*,\s*pattern\s*=\s*([^)]+)\)',
    content
)
...
if claim_reward_match:
    ...
    test_cases = ["claim_reward|mashhad", "claim_reward|tehran", "back_to_menu", "other"]
    print(f"\n   Pattern test:")
...
handler_lines = re.finditer(
    r'app\.add_handler\(CallbackQueryHandler\s*\(\s*(\w+)',
    content
)
