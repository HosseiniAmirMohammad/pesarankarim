...
GROUP_MASHHAD_PHOTO = -1004308048962  # آیدی گروه ارسال عکس مشهد
GROUP_TEHRAN_PHOTO = -1004324542601  # آیدی گروه ارسال عکس تهران
GROUP_MASHHAD_COMPLAINT = -1004383312415  # آیدی گروه نارضایتی مشهد
GROUP_TEHRAN_COMPLAINT = -1004419896463  # آیدی گروه نارضایتی تهران
...
async def admin_resend(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "📤 ارسال مجدد عکس\n\n" "لطفاً شماره تلفن ۱۱ رقمی مشتری را وارد کنید:",
...
async def handle_admin_response(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "بله، عکس دیگری دارم":
        await update.message.reply_text(
            "📸 **لطفاً عکس بعدی را ارسال کنید.**\n\n"
            f"📱 شماره: {phone}\n"
            f"🏷️ کد: {photo_code}\n\n"
            "⚠️ کپشن نیازی نیست، خودکار ارسال میشه.",
        ...
        await update.message.reply_text(
            "✅ **همه عکس‌ها ارسال شدند.**\n\n"
            f"📱 شماره: {phone}\n"
            f"🏷️ کد: {photo_code}\n"
            f"📸 تعداد عکس‌ها: {admin_data.get('count', 1)}",
            reply_markup=admin_panel_kb(),
            parse_mode="Markdown",
        )
    ...
    if context.user_data.get("admin_upload", {}).get("step") == "waiting_for_photo":
        if update.message.photo:
            ...
            await update.message.reply_text("❌ لطفاً یک عکس یا فایل بفرستید.")
            return
...
async def handle_all_messages(update, context):
            ...
            keyboard = InlineKeyboardMarkup(
                [
                    ...
                    [
                        InlineKeyboardButton(
                            "💬 ایکس (توییتر سابق)",
                            url="https://twitter.com/pesaranekarim",
                        )
                    ],
                    [
                        InlineKeyboardButton(
        ...
        if text == "دریافت عکس یادگاری":
            ...
            if branch == "tehran":
                await update.message.reply_text(
                    ...
                    "https://goo.gl/1NYXYZM5rj7QLZeUA\n\n"
                    "🔸 مسیریاب بلد (Balad):\n"
                    "https://balad.ir/p/2FnU2N9u64ujdN\n\n"
                    "🔸 مسیریاب نشان (Neshan):\n"
                    "https://nshn.ir/51_bvvEZexOWCR",
                    parse_mode="none",
                )
    ...
    if context.user_data.get("photo_step") == "code":
        if text.isdigit() and len(text) == 4:
            context.user_data["photo_code"] = text
            context.user_data["photo_step"] = "phone"
            await update.message.reply_text(
                f"کد عکس {text} ثبت شد✅\n\n"
...
async def handle_group_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    phone = lines[0].strip().replace(" ", "").replace("-", "").replace("+", "")
    photo_code = lines[1].strip()
    ...
    if result:
        ...
        try:
            if update.message.photo:
                ...
                await context.bot.send_photo(
                    chat_id=user_id,
                    photo=file_id,
                    caption=f"عکس یادگاری شما - {branch}\n\n"
        ...
        await update.message.reply_text(
            ...
            "لطفاً مطمئن شوید کاربر درخواست داده است."
        )
