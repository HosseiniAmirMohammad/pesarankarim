...
async def handle_admin_management(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("admin_action") == "add_admin":
        ...
        try:
            ...
            # ===== بررسی اینکه ورودی عددی است یا یوزرنیم =====
            if text.isdigit():
                # ورودی عددی (آیدی)
                ...
                # ورودی یوزرنیم (با @ یا بدون)
                # حذف @ اگر وجود داشت
                clean_username = text.lstrip("@")

                # بررسی اینکه یوزرنیم معتبر باشه (فقط حروف، اعداد و زیرخط)
                if not clean_username or not clean_username.replace("_", "").isalnum():
                    await update.message.reply_text(
                        "❌ یوزرنیم وارد شده معتبر نیست.\n"
                        "لطفاً یک یوزرنیم معتبر وارد کنید (مثلاً `@joe`).",
                        parse_mode="Markdown",
                    )
                    return
                ...
                try:
                    ...
                    await update.message.reply_text(
                        ...
                        "لطفاً مطمئن شوید یوزرنیم صحیح است.",
                        parse_mode="Markdown",
                    )
                    return
            ...
            if new_admin_id:
                if add_admin(new_admin_id, username, first_name, last_name, user_id):
                    # نمایش پیام موفقیت با یوزرنیم یا آیدی
