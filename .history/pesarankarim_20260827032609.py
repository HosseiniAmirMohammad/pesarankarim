...
async def handle_group_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if result:
        ...
        if file_id:
            ...
            if saved:
                await update.message.reply_text(
                    ...
                    "🔹 تا زمانی که مشتری در ربات درخواست خودش را ثبت کند، این عکس نگه داشته می‌شود و بعداً برای او ارسال خواهد شد.",
                    parse_mode="Markdown",
                )
                conn.close()
                return
        ...
        await update.message.reply_text(
            f"✅ عکس برای شماره `{phone}` و کد `{photo_code}` ذخیره شد.\n\n"
            "🔹 تا زمانی که مشتری در ربات درخواست خودش را ثبت کند، این عکس برای او ارسال خواهد شد.",
            parse_mode="Markdown",
        )
